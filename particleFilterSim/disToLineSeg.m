function [ distances_t ] = disToLineSeg(point,line )
...
distances_t = zeros(length(t),1);
for i=1:length(t)
    if t(i) <0
        distances_t(i) = distance(point(i,:),lStart(i,:));
    elseif t(i) >1
        distances_t(i) = distance(point(i,:),lEnd(i,:));
    else
        projection = lStart(i,:) + t(i) * (vec(i,:));
        distances_t(i) = distance(point(i,:), projection);
    end
end
end
